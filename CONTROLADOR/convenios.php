...
if(isset($_SESSION['user_session']) == 'afiliado_session') {
  ...
  if($id_empresa_conv == 1) {
    $url = $dirRoot."VISTA/VI_CONVENIOS/convenios_empresa_1.php";
    header("Location: ".$url);
  }elseif($id_empresa_conv == 2 ){
    $url = $dirRoot."VISTA/VI_CONVENIOS/convenios_empresa_2.php";
    header("Location: ".$url);
  }elseif($id_empresa_conv == 3 ){
    $url = $dirRoot."VISTA/VI_CONVENIOS/convenios_empresa_3.php";
    header("Location: ".$url);
  }elseif($id_empresa_conv == 4 ){
    $url = $dirRoot."VISTA/VI_CONVENIOS/convenios_empresa_4.php";
    header("Location: ".$url);
    ...
    $url = $dirRoot."VISTA/VI_CONVENIOS/convenios_empresa_7.php";
    header("Location: ".$url);
  }else {
    $url = $dirRoot."VISTA/VI_CONVENIOS/convenios_empresa_noexiste.php";
    header("Location: ".$url);
  }
...
<body class="cuerpo_contenedor<?php echo $sufijo_estilo; ?>" >
    ...
    <footer class="pie<?php echo $sufijo_estilo; ?>"><?php include $incRoot."VISTAVI_INCLUDES/pie.php" ?></footer>
