...
<?php 
		...
		if (isset($nombre)){
			...
			if ($dat->existeAdmin($nombre) && $dat->verificaPassAdmin($nombre,$pass)) {
				...
				$url = $dirRoot.'VISTA/VI_ADMINISTRADORES/vistaAdminMenu.php';
				header('Location: '.$url);
			
		} else {
			//Si no mostramos un mensaje de no encontrado y un link a index
			$url = $dirRoot.'VISTA/user_pass_error.php?tipologin=admin';
			header('Location: '.$url);
		}
	} else {
		//Si no mostramos un mensaje de no encontrado y un link a index
		$url = $dirRoot.'VISTA/user_noencontrado_error.php?tipologin=admin';
		header('Location: '.$url);
	}
	?>
</body>
